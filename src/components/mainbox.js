...
export default function Mainbox (props) {
    ...
    function handleMouseOut(theclass) {
      document.querySelector("."+theclass).classList.remove("animated","pulse");
    }
...
var innerdata = [];
    ...
    props.myinfo.map((elem) => {
      ...
      innerdata.preview = elem.preview.map((previewitem) => {
        return ReactHtmlParser("<li>" + previewitem + "</li>");
      });
      return innerdata;
    });
  ...
  return ( 
    <Router>
  <div className="white-bg" onClick={() => {props.router.history.replace(`/${innerdata.class}`)}}><div
